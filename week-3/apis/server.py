...
def find_afterparties():
    ...
    if location and distance and measurement:
        ...
        payload = {'token': os.environ["EVENTBRITE_TOKEN"], 'q':query,
                   'location.address':location,'location.within':distance,'sort_by':sort }
...
def create_eventbrite_event():
    ...
    payload = {'event.name.html': name,'event.start.utc': start_time, 'event.end.utc': end_time, 
               'event.start.timezone': timezone, 'event.end.timezone': timezone, 'event.currency': currency}

    headers = {'Authorization': 'Bearer ' + os.environ["EVENTBRITE_TOKEN"]}
    response = requests.post('https://www.eventbriteapi.com/v3/events/', data=payload, headers=headers)
...
def show_my_events():
    ...
    response = requests.get('https://www.eventbriteapi.com/v3/users/me/events/', params=payload)
